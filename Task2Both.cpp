        ...
        for (int t = 0; t <= maxTime; ++t) {
            ...
            for (int j = 0; j < n; ++j) {
                ...
                if (startTime < jobs[j].d) {
                    finishTime = startTime + jobs[j].a;
                } else {
                    finishTime = startTime + jobs[j].a - jobs[j].b;
                }

                // Check bounds
                if (finishTime > maxTime || finishTime < 0) {
                    // Uncomment if you want to log it:
                    // cout << "Skipping out-of-bound finishTime=" << finishTime << " at mask=" << newMask << endl;
                    continue;
                }
...
//
// Created by minne on 20/04/2025.
//
